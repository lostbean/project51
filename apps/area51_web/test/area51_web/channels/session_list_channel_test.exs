defmodule Area51Web.SessionListChannelTest do
  # Changed to async: false to avoid mocking conflicts
  use Area51Web.LiveStateChannelCase, async: false

  alias Area51Web.SessionListChannel
  alias Area51Web.Auth.Guardian

  @endpoint Area51Web.Endpoint
  ...
  setup do
    ...
    Code.ensure_loaded(Area51Data.GameSession)

    # Mock Area51Data.GameSession.list_sessions_for_ui
    :meck.new(Area51Data.GameSession, [:passthrough])

    :meck.expect(Area51Data.GameSession, :list_sessions_for_ui, fn ->
      [
    ...
    # Create test socket
    socket = Phoenix.ChannelTest.socket(Area51Web.LiveStateSocket, %{}, %{})
  ...
  describe "handling events" do
    ...
    test "create_session handles success case", %{joined_socket: socket} do
      ...
      :meck.expect(Area51Data.GameSession, :create_game_session, fn _mystery_data ->
        :ok
      end)
