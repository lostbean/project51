defmodule Area51Gleam do
  @moduledoc """
  Documentation for `Area51Gleam`.
  """
  ...
  defmodule State do
    ...
    defmodule Navigation do
      ...
      @type t() :: %__MODULE__{
              investigations: list(Area51Gleam.InvestigationCard.t())
            }
    end
    ...
    def from_gleam({:investigation_state, id, clues, goal, title}) do
      %InvestigationState{
        id: id,
        clues: Enum.map(clues, &Area51Gleam.Clue.from_gleam/1),
        goal: goal,
        title: title
      }
    end
    ...
    def from_gleam({:navigation, investigations}) do
      %Navigation{
        investigations: Enum.map(investigations, &Area51Gleam.InvestigationCard.from_gleam/1)
      }
    end
    ...
    def to_gleam(%InvestigationState{id: id, clues: clues, goal: goal, title: title}) do
      {:investigation_state, id, Enum.map(clues, &Area51Gleam.Clue.to_gleam/1), goal, title}
    end

    def to_gleam(%Navigation{investigations: investigations}) do
      {:navigation, Enum.map(investigations, &Area51Gleam.InvestigationCard.to_gleam/1)}
    end
